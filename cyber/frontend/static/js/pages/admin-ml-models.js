const AdminMLModelsPage = {
  ...
  async render() {
    ...
    try {
      ...
      const [modelsData, metricsData] = await Promise.all([
        ApiClient.getMLModels({ signal: this._abortController.signal }).catch(() => []),
        ApiClient.getMLMetrics({ signal: this._abortController.signal }).catch(() => null),
      ]);
      this._models = Array.isArray(modelsData) ? modelsData : (modelsData.results || []);
      this._metrics = metricsData;
  ...
  _template() {
    ...
    const metricsHtml = this._metrics ? `
      <div class="mb-6 p-4 bg-zinc-900/60 border border-emerald-500/30 rounded-xl space-y-4 font-mono text-xs">
        <div class="flex justify-between items-center border-b border-zinc-800 pb-2">
          <span class="text-emerald-400 font-bold uppercase tracking-wider text-sm">🛡️ Production ML Verification & Audit Panel</span>
          <span class="text-zinc-400">Split: ${this._metrics.split_strategy || 'Temporal 80/20'}</span>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="bg-black/40 p-3 rounded border border-zinc-800">
            <span class="text-zinc-500 uppercase text-[10px] block">Top-1 Accuracy</span>
            <span class="text-xl font-bold text-emerald-400">${(this._metrics.top_1_accuracy * 100).toFixed(1)}%</span>
          </div>
          <div class="bg-black/40 p-3 rounded border border-zinc-800">
            <span class="text-zinc-500 uppercase text-[10px] block">Top-3 Accuracy</span>
            <span class="text-xl font-bold text-emerald-400">${(this._metrics.top_3_accuracy * 100).toFixed(1)}%</span>
          </div>
          <div class="bg-black/40 p-3 rounded border border-zinc-800">
            <span class="text-zinc-500 uppercase text-[10px] block">Top-5 Accuracy</span>
            <span class="text-xl font-bold text-emerald-400">${(this._metrics.top_5_accuracy * 100).toFixed(1)}%</span>
          </div>
          <div class="bg-black/40 p-3 rounded border border-zinc-800">
            <span class="text-zinc-500 uppercase text-[10px] block">Log Loss</span>
            <span class="text-xl font-bold text-cyan-400">${this._metrics.log_loss ? this._metrics.log_loss.toFixed(4) : 'N/A'}</span>
          </div>
        </div>
        <div class="flex flex-wrap gap-4 text-zinc-400 pt-1 text-[11px] border-t border-zinc-800/60">
          <span>Classifier: <strong class="text-zinc-200">${this._metrics.classifier || 'LightGBM'}</strong></span>
          <span>Clean Features: <strong class="text-zinc-200">${this._metrics.num_features || 33}</strong></span>
          <span>Train Samples: <strong class="text-zinc-200">${this._metrics.train_samples || 0}</strong></span>
          <span>Test Samples: <strong class="text-zinc-200">${this._metrics.test_samples || 0}</strong></span>
          <span>Trained: <strong class="text-zinc-200">${this._metrics.trained_at ? new Date(this._metrics.trained_at).toLocaleString() : 'N/A'}</strong></span>
        </div>
      </div>
    ` : '';
    ...
    return `
      <div class="mb-6 flex justify-between items-end">
        <div>
          <h1 class="text-3xl font-bold font-mono text-white mb-2">ML Models & Verification</h1>
          <p class="text-zinc-500 font-mono text-sm">Model registry, non-deceptive evaluation metrics, and drift monitoring.</p>
        </div>
