        ...
        html = `
            ...
            <form id="login-form" onsubmit="handleLogin(event)" class="space-y-4">
                <div>
                    ...
                    <input type="email" id="login-email" name="email" class="w-full bg-black border border-emerald-900/50 rounded p-2 text-white font-mono text-sm focus:border-emerald-500 focus:outline-none" required value="analyst@cyber.io">
                </div>
                <div>
                    <label class="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">Password</label>
                    <input type="password" id="login-password" name="password" class="w-full bg-black border border-emerald-900/50 rounded p-2 text-white font-mono text-sm focus:border-emerald-500 focus:outline-none" required value="demo1234">
                </div>
...
async function handleLogin(e) {
    ...
    if (typeof Validator !== 'undefined') {
        ...
        const { valid, errors } = Validator.validate({ email, password }, schema);
        if (!valid) {
            Validator.showErrors('login-form', errors);
    ...
    try {
        ...
        errorEl.textContent = err.message || 'Network error connecting to auth server.';
        errorEl.classList.remove('hidden');
    }
}
